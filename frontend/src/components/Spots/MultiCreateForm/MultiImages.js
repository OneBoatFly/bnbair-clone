...
export default function MultiImages({ formData, setFormData, hasSubmitted, newSpot }) {
  ...
  const handleSubmit = (e) => {
    ...
    imageUpload.forEach((image, idx) => {
      const imageRef = ref(storage, `spots/${newSpot.id}/${image.name + v4()}`)
  ...
  const handleDelete = (img, idx) => {
    ...
    if (img.id && img.url.includes('firebase')) {
      ...
      deleteObject(imageRef).then(() => {
        ...
        dispatch(spotIamgesActions.deleteImage(img.id, newSpot.id))
      }).catch((e) => {
  ...
  useEffect(() => {
    ...
    const existingImages = newSpot.SpotImages || [];
