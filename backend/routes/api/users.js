...
// list of middlewares to check username
// (check length, and check not email), email, password, then validate them
const validateSignup = [
  check('email', 'Valid email is required.').exists({ checkFalsy: true }).isEmail(),
  check('username', 'Please provide a username with at least 4 characters.')
    .exists({ checkFalsy: true })
    .isLength({ min: 4 }),
  check('username', 'Username is required.').not().isEmpty({ ignore_whitespace: true }),
  ...
  check('firstName', 'First name is required.').not().isEmpty({ ignore_whitespace: true }),
  check('lastName').exists({ checkFalsy: true }).withMessage('Last name is required.'),
  check('lastName', 'Last name is required.').not().isEmpty({ ignore_whitespace: true }),
...
router.post('/', validateSignup, async (req, res, next) => {
  let {
    username, email, firstName, lastName, password, profileUrl, isSuperhost,
  } = req.body;
