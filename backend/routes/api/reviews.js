...
const {
  Review, ReviewImage, User, Spot, SpotImage,
} = require('../../db/models');
...
router.get('/current', requireAuth, async (req, res) => {
  ...
  for (let i = 0; i < reviews.length; i += 1) {
    const reviewJSON = reviews[i].toJSON();
...
const validateReviewImage = [
  check('url').exists({ checkFalsy: true }).isURL().withMessage('Image url must be provided.'),
...
router.post('/:reviewId/images', requireAuth, validateReviewImage, async (req, res, next) => {
  ...
  if (!review) {
    ...
    err.status = 404;
    next(err);
  } else if (review.toJSON().ReviewImages.length === 10) {
